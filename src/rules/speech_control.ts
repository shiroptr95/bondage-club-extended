...
export function initRules_bc_speech_control() {
	...
	registerRule("speech_restrict_beep_receive", {
		...
		longDescription: "Блокирует PLAYER_NAME от получения каких-либо звуковых сигналов (независимо, содержит звуковой сигнал сообщение или нет), за исключением сигналов от участника из списка. Если кто-то попытается отправить PLAYER_NAME звуковое сообщение, пока это правило активно, они получают звуковой сигнал автоответа, если включен автоответчик. PLAYER_NAME не получит никаких указаний на то, что она получила бы звуковой сигнал, если бы правило не применялось; в этом случае она увидит и звуковой сигнал, и автоматический ответ. Правило можно настроить так, чтобы оно активировалось только когда PLAYER_NAME не может пользоваться руками (например, привязан к кресту).",
